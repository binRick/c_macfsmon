...
void watchful_monitor_deinit(WatchfulMonitor *wm) {
  ...
  if (NULL != wm->excludes) {
    for (size_t i = 0; i < wm->excludes->len; i++) {
      if (NULL != wm->excludes->paths[i]) {
        free(wm->excludes->paths[i]);
      }
    }
    if (NULL != wm->excludes->paths) {
      free(wm->excludes->paths);
    }
    free(wm->excludes);
  }
...
bool watchful_monitor_excludes_path(WatchfulMonitor *wm, const char *path) {
  ...
  for (size_t i = 0; i < wm->excludes->len; i++) {
    ...
    if (wildmatch(exclude, path, WM_WILDSTAR) == WM_MATCH) {
      return(true);
    }
  }
  return(false);
