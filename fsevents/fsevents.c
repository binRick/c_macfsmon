...
static bool is_historical_event(WatchfulMonitor *wm, const char *path, int flag, int event) {
  ...
  if (event == WATCHFUL_EVENT_CREATED) {
    ...
    if (timecmp(wm->start_time, &st.st_birthtimespec) >= 0) {
      return(true);
    }
    ...
    if (NULL != val && timecmp(val, &st.st_birthtimespec) >= 0) {
      return(true);
    }
...
static void handle_event(ConstFSEventStreamRef         streamRef,
  ...
  for (size_t i = 0; i < numEvents; i++) {
    ...
    if (!(event_type && (wm->events & event_type))) {
      continue;
    }
